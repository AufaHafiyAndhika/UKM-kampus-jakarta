...
@section('title', 'Anggota UKM ' . $ukm->name)
            ...
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        ...
                        <dd class="text-lg font-medium text-gray-900">{{ number_format($activeMembers) }}</dd>
                    </dl>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        ...
                        <dd class="text-lg font-medium text-gray-900">{{ number_format($pendingMembers) }}</dd>
                    </dl>
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center">
                ...
                <div class="ml-5 w-0 flex-1">
                    <dl>
                        ...
                        <dd class="text-lg font-medium text-gray-900">{{ number_format($rejectedMembers) }}</dd>
                    </dl>
                </div>
            </div>
        </div>
...
<div class="bg-white shadow rounded-lg">
    ...
    <div class="px-6 py-4 border-b border-gray-200">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-semibold text-gray-900">Anggota UKM {{ $ukm->name }}</h1>
                <p class="text-sm text-gray-600">Kelola anggota Unit Kegiatan Mahasiswa</p>
            </div>
    ...
    <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
        <form method="GET" class="flex flex-wrap items-center gap-4">
            ...
            @if(request()->hasAny(['search', 'status', 'role']))
                <a href="{{ route('admin.ukms.members', $ukm) }}" 
                   class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg font-medium transition-colors">
                    Reset
                </a>
            @endif
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                    {{ $member->pivot->role === 'leader' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {{ $member->pivot->joined_date ? $member->pivot->joined_date->format('d M Y') : '-' }}
                            </td>
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-4">
                    <p class="text-sm text-gray-700">
                        Menampilkan
                        <span class="font-medium">{{ $members->firstItem() ?? 0 }}</span>
                        sampai
                        <span class="font-medium">{{ $members->lastItem() ?? 0 }}</span>
                        dari
                        <span class="font-medium">{{ number_format($members->total()) }}</span>
                        anggota
                        @if(request()->hasAny(['search', 'status', 'role']))
                            <span class="text-gray-500">(difilter dari {{ number_format($totalMembers) }} total)</span>
                        @endif
                    </p>
                </div>
                <div>
    ...
    @else
        <div class="text-center py-12">
            ...
            <p class="mt-1 text-sm text-gray-500">
                ...
                @else
                    UKM ini belum memiliki anggota.
                @endif
            </p>
        </div>
    @endif
